...
// export const getValidatorProfileUrl = async (identity) => {
//     console.log("Get validator avatar.")
//     if (identity.length == 16) {
//         try {
//             await fetch(`https://keybase.io/_/api/1.0/user/lookup.json?key_suffix=${identity}&fields=pictures`)
...
getValidatorUptime = async (validatorSet) => {
    ...
    console.log("Get validator uptime")
    ...
    try {
        ...
        console.log(await fetch(url))
            .then(function (response) {
                if (response.ok) {
                    response.json().then((data) => {
                        Chain.upsert({ chainId: Meteor.settings.public.chainId }, { $set: { "slashing": data } });
                    })
                }
                else {
                    console.log(JSON.stringify(response))
        ...
        try {
            ...
            await fetch(url)
                .then(function (response) {
                    if (response.ok) {
                        response.json().then((data) => {
                            let signingInfo = data.val_signing_info;
                            if (signingInfo) {
...
Meteor.methods({
    ...
    'blocks.getLatestHeight': async function () {
        this.unblock();
        let url = RPC + '/status';
        try {
            await fetch(url)
                ...
                // .then(data => {
                //     console.log("DATA" )
                //     console.log(data.result.sync_info.latest_block_height)
                    ...
                    for (v in validatorSet) {
                        ...
                        if (!valExist && valData.consensus_pubkey) {
                            ...
                            if (valData.description && valData.description.identity) {
                                try {
                                    valData.profile_url = await getValidatorProfileUrl(valData.description.identity)
                                }
                                catch (e) {
