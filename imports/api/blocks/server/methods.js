...
getValidatorProfileUrl = (identity) => {
    if (identity.length == 16) {
        ...
        if (response.statusCode == 200) {
            ...
            return (
                them && them.length && them[0].pictures && them[0].pictures.primary && them[0].pictures.primary.url
            )
        } else {
...
Meteor.methods({
    ...
    'kafka.update': async function (message) {
        this.unblock()
    ...
    'blocks.blocksUpdate': function () {
        ...
        if (until > curr) {
                    ...
                    if (response.statusCode == 200) {
                        ...
                        if (block.block.data.txs && block.block.data.txs.length > 0) {
                            for (t in block.block.data.txs) {
                                Meteor.call(
                                    'Transactions.index',
                                    sha256(Buffer.from(block.block.data.txs[t], 'base64')),
                                    blockData.time,
                                    (err, result) => {
                                        if (err) {
                                            console.log(err)
                        ...
                        console.log(
                            'Block insert time: ' + (endBlockInsertTime - startBlockInsertTime) / 1000 + 'seconds.'
                        )
                                ...
                                if (height % 15 == 0) {
                                    ...
                                    if (record.exists) {
                                        ...
                                        bulkValidators
                                            ...
                                            .upsert()
                                            .updateOne({ $set: { uptime: uptime, lastSeen: blockData.time } })
                                    } else {
                                        uptime = (uptime / base) * 100
                                        bulkValidators
                                            .find({ address: address })
                                            .upsert()
                                            .updateOne({ $set: { uptime: uptime } })
                                    }
                                }
                        ...
                        console.log(
                            'Get height validators time: ' +
                (endGetValidatorsTime - startGetValidatorsTime) / 1000 +
                'seconds.'
                        )

                        Chain.update(
                            { chainId: block.block.header.chain_id },
                            { $set: { lastSyncedTime: blockData.time, blockTime: blockTime } }
                        )
                        ...
                        if (validators.result) {
                            ...
                            for (v in validators.result.validators) {
                                ...
                                if (!valExist) {
                                    ...
                                    validator.consensus_pubkey = Meteor.call(
                                        'pubkeyToBech32',
                                        validator.pub_key,
                                        Meteor.settings.public.bech32PrefixConsPub
                                    )
                      ...
                      (!valExist.description ||
                        validatorData.description.identity !== valExist.description.identity)
                                        ...
                                        if (height % 30 == 1) {
                                            try {
                                                let response = HTTP.get(
                                                    LCD +
                            '/staking/delegators/' +
                            valExist.delegator_address +
                            '/delegations/' +
                            valExist.operator_address
                                                )

                                                if (response.statusCode == 200) {
                                                    let selfDelegation = JSON.parse(response.content).result
                                                    if (selfDelegation.shares) {
                                                        validator.self_delegation =
                              parseFloat(selfDelegation.shares) / parseFloat(validator.delegator_shares)
                                                    }
                                                }
                                            } catch (e) {
                                                // console.log(e);
                                        ...
                                        bulkValidators
                                            .find({ consensus_pubkey: valExist.consensus_pubkey })
                            ...
                            if (prevValidators) {
                                let removedValidators = getRemovedValidators(
                        ...
                        if (height % 14400 == 0) {
                            try {
                                ...
                                let dbValidators = {}
                                Validators.find({}, { fields: { consensus_pubkey: 1, status: 1 } }).forEach(
                                    (v) => (dbValidators[v.consensus_pubkey] = v.status)
                                )
                                Object.keys(validatorSet).forEach((conPubKey) => {
                                    ...
                                    if (dbValidators[conPubKey] == undefined) {
                                        ...
                                        let pubkeyType = Meteor.settings.public.secp256k1
                                        ...
                                        validatorData.delegator_address = Meteor.call(
                                            'getDelegator',
                                            validatorData.operator_address
                                        )

                                        validatorData.accpub = Meteor.call(
                                            'pubkeyToBech32',
                                            validatorData.pub_key,
                                            Meteor.settings.public.bech32PrefixAccPub
                                        )
                                        validatorData.operator_pubkey = Meteor.call(
                                            'pubkeyToBech32',
                                            validatorData.pub_key,
                                            Meteor.settings.public.bech32PrefixValPub
                                        )
                                        ...
                                        bulkValidators
                                            .find({ consensus_pubkey: conPubKey })
                                            .upsert()
                                            .updateOne({ $set: validatorData })
                                ...
                                try {
                                    ...
                                    if (profileUrl) {
                                        bulkValidators
                                            ...
                                            .upsert()
                                            .updateOne({ $set: { profile_url: profileUrl } })
                                    }
                                } catch (e) {
                        ...
                        console.log(
                            'Get validators name time: ' +
                (endFindValidatorsNameTime - startFindValidatorsNameTime) / 1000 +
                'seconds.'
                        )
                        ...
                        Meteor.call('kafka.update', analyticsData)

                        let endAnalyticsInsertTime = new Date()
                        console.log(
                            'Analytics insert time: ' +
                (endAnalyticsInsertTime - startAnayticsInsertTime) / 1000 +
                'seconds.'
                        )
            ...
            Chain.update(
                ...
                { $set: { lastBlocksSyncedTime: new Date(), totalValidators: totalValidators } }
            )
        }
