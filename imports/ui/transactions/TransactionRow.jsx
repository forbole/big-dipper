...
export const TransactionRow = (props) => {
    ...
    return <SentryBoundary><Row className={(tx.code)?"tx-info w-40 invalid":"tx-info w-40"}>
        ...
        <Col xs={(!props.blockList)?6:8} md={(!props.blockList)?9:4} lg={2} className="fee"><i className="material-icons d-lg-none">monetization_on</i> {(tx.tx.auth_info.fee.amount.length > 0)?tx.tx.auth_info.fee.amount.map((fee,i) => {
            console.log(fee.amount);
            return <span className="text-nowrap" key={i}>{(new Coin(fee.amount, (fee)?fee.denom:null)).stakeString()}</span>
