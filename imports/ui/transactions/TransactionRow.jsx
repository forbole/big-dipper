...
export const TransactionRow = (props) => {
    ...
    return <SentryBoundary><Row className={(tx.code)?"tx-info invalid":"tx-info"}>
        ...
        {(!props.blockList)?<Col xs={6} md={9} lg={{size:2,order:"last"}} className="text-nowrap"><i className="material-icons">schedule</i> <span>{tx.block()?<TimeAgo time={tx.block().time} />:''}</span>{(tx.tx.value.memo && tx.tx.value.memo != "")?<span>
