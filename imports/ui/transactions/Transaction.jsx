                        ...
                        <CardBody>
                            <Row>
                                ...
                                <Col md={8} className="value">{(tx.tx.auth_info.fee.amount.length > 0)?tx.tx.auth_info.fee.amount.map((fee,i) => {
                                    return <span className="text-nowrap" key={i}> {((fee.amount/Meteor.settings.public.stakingFraction)>=1)?(new Coin(fee.amount, fee.denom)).stakeString():(new Coin(fee.amount, fee.denom)).mintString()} </span>
