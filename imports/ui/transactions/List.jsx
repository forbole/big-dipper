...
const TransactionRow = (props) => {
    ...
    return <Row className="tx-info">
        ...
        <Col xs={6} md={2} lg={4} className="text-truncate"><Link to="#">{tx.hash}</Link></Col>
        <Col xs={{size:2}} md={{size:2, order:"last"}} lg={1}>{(!tx.result.code)?<TxIcon valid />:<TxIcon />}</Col>
        <Col xs={{order:11}} md={{size:6}}>{(tx.tx.value.msg && tx.tx.value.msg.length >0)?tx.tx.value.msg.map((msg,i) => {
            return <span key={i} >
                <span id={"tx-"+tx.hash+"-"+i}>
                    <MsgType type={msg.type}/>
                </span>
                <UncontrolledTooltip placement="top" target={"tx-"+tx.hash+"-"+i}>
