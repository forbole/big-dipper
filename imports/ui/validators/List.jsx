...
const ValidatorRow = (props) => {
    ...
    return <Card body>
        <Row className="validator-info">
            ...
            {(!props.jailed)?<Col className="commission data" xs={{size:5, offset:2}} md={{size:1,offset:0}} lg={2}><i className="material-icons d-sm-none">call_split</i> <span>{numeral(props.validator.commission.rate).format('0.00%')}</span></Col>:''}
            {(!props.jailed)?<Col className="uptime data" xs={{size:4,order:"last"}} md={2}><Progress animated value={props.validator.uptime}>{props.validator.uptime?props.validator.uptime.toFixed(2):0}%</Progress></Col>:''}
