...
const ValidatorRow = (props) => {
    ...
    return <Card body>
        <Row className="validator-info">
            ...
            <Col className="voting-power data" xs={{size:4, offset:2}} md={{size:3, offset:0}} lg={2}><i className="material-icons d-md-none">power</i>  <span>{numeral(props.validator.voting_power).format('0,0')}</span><span className="d-none d-md-inline"> ({numeral(props.validator.voting_power/props.totalPower*100).format('0.00')}%)</span></Col>
            ...
            {(!props.jailed)?<Col className="comission data" xs={{size:5, offset:2}} md={{size:2,offset:0}} lg={2}><i className="material-icons d-sm-none">call_split</i> <span>{numeral(props.validator.commission.rate).format('0.00%')}</span></Col>:''}
            {(!props.jailed)?<Col className="uptime data" xs={4} md={1} lg={2}><Progress animated value={props.validator.uptime}>{props.validator.uptime?props.validator.uptime.toFixed(2):0}%</Progress></Col>:''}
