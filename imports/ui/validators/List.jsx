...
const ValidatorRow = (props) => {
    ...
    return <Card body>
        <SentryBoundary><Row className="validator-info">
            ...
            <Col xs={12} md={2} className="data"><Link to={"/validator/"+props.validator.operatorAddress}><Avatar moniker={moniker} profileUrl={props.validator.profile_url} address={props.validator.address} list={true} /><span className="moniker">{moniker}</span></Link></Col>
            ...
            {(!props.inactive)?<Col className="commission data" xs={{size:4}} md={{size:1,offset:0}} lg={2}><i className="material-icons d-sm-none">call_split</i> <span>{(props.validator.commission&&props.validator.commission.commissionRates)?numbro(props.validator.commission.commissionRates.rate/Meteor.settings.public.humanizeReduction).format('0.00%'):''}</span></Col>:''}
