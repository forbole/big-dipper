...
export default class Proposal extends Component{
    ...
    componentDidUpdate(prevProps){
        if (this.props.proposal != prevProps.proposal){
            ...
            let now = moment();
            if (this.props.proposal.value.voting_start_time != '0001-01-01T00:00:00Z'){
                if (now.diff(moment(this.props.proposal.value.voting_start_time)) > 0){
                    ...
                    if (now.diff(endVotingTime) < 0){
                        ...
                        this.setState({
                            ...
                            voteEnded: true,
                            totalVotes: totalVotes,
                            yesPercent: (totalVotes>0)?parseInt(this.props.proposal.value.final_tally_result.yes)/totalVotes*100:0,
                            abstainPercent: (totalVotes>0)?parseInt(this.props.proposal.value.final_tally_result.abstain)/totalVotes*100:0,
                            noPercent: (totalVotes>0)?parseInt(this.props.proposal.value.final_tally_result.no)/totalVotes*100:0,
                            noWithVetoPercent: (totalVotes>0)?parseInt(this.props.proposal.value.final_tally_result.no_with_veto)/totalVotes*100:0,
                            proposalValid: (this.state.totalVotes/this.props.chain.totalVotingPower > parseFloat(this.props.chain.gov.tallyParams.quorum))?true:false
                        })
                    }
                }
    ...
    render(){
        if (this.props.loading){
            ...
            if (this.props.proposalExist && this.state.proposal != ''){
                ...
                return <div>
                    <Helmet>
                        <title>{this.state.proposal.title} | The Big Dipper</title>
                        <meta name="description" content={this.state.proposal.description} />
                    </Helmet>
                    <div className="proposal bg-light">
                        ...
                        <Row className="mb-2 border-top">
                            ...
                            <Col md={9} className="value">{this.state.proposal.proposal_type.substr(4).match(/[A-Z]+[^A-Z]*|[^A-Z]+/g).join(" ")}</Col>
                        </Row>
                        <Row className="mb-2 border-top">
                            <Col md={3} className="label"><T>proposals.proposalStatus</T></Col>
                            <Col md={9} className="value"><ProposalStatusIcon status={this.state.proposal.proposal_status} /> {(this.props.proposal.proposal_status)?this.props.proposal.proposal_status.match(/[A-Z]+[^A-Z]*|[^A-Z]+/g).join(" "):''}</Col>
                        </Row>
                        <Row className="mb-2 border-top">
