...
export default class AccountDetails extends Component {
        ...
        if (result) {
          ...
          if (result.delegations && result.delegations.length > 0) {
            ...
            this.state.total.forEach((total, i) => {
              if (total.denom === Meteor.settings.public.bondDenom)
                this.state.total[i].amount =
                  parseFloat(this.state.total[i].amount) +
                  parseFloat(this.state.delegated);
            }, this);
          ...
          if (result.unbonding && result.unbonding.length > 0) {
            ...
            this.state.total.forEach((total, i) => {
              if (total.denom === Meteor.settings.public.bondDenom)
                this.state.total[i].amount =
                  parseFloat(this.state.total[i].amount) +
                  parseFloat(this.state.unbonding);
            }, this);
          ...
          if (result.total_rewards && result.total_rewards.length > 0) {
            ...
            totalRewards.forEach((rewardNum, i) => {
              this.state.total.find(({ denom }) => denom === rewardNum.denom)
                ? (this.state.total[i].amount =
                    parseFloat(this.state.total[i].amount) +
                    parseFloat(rewardNum.amount))
                : this.state.total.push(rewardNum);
            }, this);
          ...
          if (result.rewards && result.rewards.length > 0) {
            for (let c = 0; c < result.rewards.length; c++) {
              if (result.rewards[c].reward != null) {
                numRewards[result.rewards[c]['validator_address']] =
                  result.rewards[c].reward;
              }
            }
          ...
          if (result.commission) {
            result.commission.forEach((commissions, i) => {
              ...
              this.state.total.find(({ denom }) => denom === commissions.denom)
                ? (this.state.total[i].amount =
                    parseFloat(this.state.total[i].amount) +
                    parseFloat(commissions.amount))
                : this.state.total.push(commissions);
  ...
  displayStakingDenom(denomType) {
    let findCoinType = Meteor.settings.public.coins.find(
      ({ denom }) => denom === denomType
    );
  ...
  renderDropDown() {
    return (
      ...
      >
        ...
        <DropdownMenu>
          {this.state.total.map((option, k) => (
            <DropdownItem
              key={k}
              onClick={e => this.handleCoinSwitch(option.denom, e)}
            >
              {this.displayStakingDenom(option.denom)}
            </DropdownItem>
          ))}
        </DropdownMenu>
      </UncontrolledDropdown>
  ...
  renderShareLink() {
    ...
    let otherLinks = [{ label: 'Transfer', url: `${primaryLink}/send` }];
    return <LinkIcon link={primaryLink} otherLinks={otherLinks} />;
  }

  findCoin(coins) {
    let finder = coins.find(({ denom }) => denom === this.state.denom);
    let coinFinder = finder
      ? new Coin(finder.amount, finder.denom).toString(4)
      : '0.0000 ' + this.state.denom;
    return coinFinder;
  }
  ...
  checkIfBondDenom(parameter) {
    if (parameter === Meteor.settings.public.bondDenom) return true;
    else return false;
  }

  render() {
    let findCurrentCoin = this.state.total.find(
      ({ denom }) => denom === this.state.denom
    );
    ...
    if (this.state.loading) {
      ...
      return (
        <div id="account">
          <Helmet>
            <title>
              Account Details of {this.state.address} on Cosmos Hub | The Big
              Dipper
            </title>
            <meta
              name="description"
              content={
                'Account Details of ' + this.state.address + ' on Cosmos Hub'
              }
            />
          </Helmet>
          <Row>
            ...
            <Col>
              <Card>
                <CardHeader>
                  ...
                  {this.state.available.length > 1 ? (
                    <div className="coin-dropdown float-right">
                      <h5>Select Coin:</h5> {this.renderDropDown()}
                    </div>
                  ) : null}
                </CardHeader>
                <CardBody>
                    ...
                    <Col xs={12}>
                      <Progress multi>
                        <Progress
                          ...
                          className="available"
                          value={
                            (this.findValue(this.state.available) /
                              currentCoinTotal) *
                            100
                          }
                        />
                        <Progress
                          ...
                          className="delegated"
                          value={
                            (this.state.delegated / currentCoinTotal) * 100
                          }
                        />
                        <Progress
                          bar
                          className="unbonding"
                          value={
                            (this.state.unbonding / currentCoinTotal) * 100
                          }
                        />
                        <Progress
                          bar
                          className="rewards"
                          value={
                            (this.findValue(this.state.rewards) /
                              currentCoinTotal) *
                            100
                          }
                        />
                        <Progress
                          ...
                          value={
                            (this.findValue(this.state.commission) /
                              currentCoinTotal) *
                            100
                          }
                        />
                  ...
                  <Row>
                    <Col md={6} lg={8}>
                      ...
                      <Row>
                        ...
                        <Col xs={8} className="value text-right">
                          {this.checkIfBondDenom(this.state.denom)
                    ...
                    <Col
                      md={6}
                      lg={4}
                      className="total d-flex flex-column justify-content-end"
                    >
                      {this.state.user ? (
                        <Row>
                          <Col xs={12}>
                      ...
                      <Row>
                        ...
                        <Col
                          xs={12}
                          className="dollar-value text-right text-secondary"
                        >
                          ~
                          {numbro(
                            (this.findValue(this.state.total) /
                              Coin.StakingCoin.fraction) *
                              this.state.price
                          ).format('$0,0.0000a')}{' '}
