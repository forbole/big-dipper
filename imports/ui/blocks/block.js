    ...
    render() {
        ...
        if (proposer){
            ...
            return <Row className="block-info">
                    ...
                    <Col xs={4} sm={1} md={2}><i className="fas fa-sync d-sm-none"></i> {numeral(this.props.block.transNum).format('0,0')}</Col>
                    <Col xs={{size:4, offset:8}} sm={{size:2, offset:0}}><i className="fas fa-database d-sm-none"></i> <Link to={"/blocks/"+this.props.block.height}>{numeral(this.props.block.height).format('0,0')}</Link></Col>
                </Row>    
        }
        else{
