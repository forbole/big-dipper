...
export default class Block extends Component {
    ...
    render() {
        if (this.props.block){
            let proposerAddress = this?.props?.block?.proposerAddress
            let validator = Validators.findOne({ address: proposerAddress});
            let moniker = validator?.description?.moniker ??  this.props.block.proposerAddress;
            return <Row className="block-info">
                ...
                <Col xs={8}sm={3} md={2} lg={3} className="text-truncate"><Link to={"/validator/"+this.props.block.proposerAddress}><Avatar moniker={moniker} profileUrl={this.props.block.profile_url} address={this.props.block.proposerAddress} list={true} /> {moniker}</Link></Col>
