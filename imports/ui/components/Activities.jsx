        ...
        case "cosmos-sdk/MsgSend":
            ...
            for (let a in msg.value.amount) {
                if (a > 0) {
                    amount +=
                            ", " +
                            numbro(msg.value.amount[a].amount).format("0,0") +
                            " " +
                            msg.value.amount[a].denom;
                } else {
                    amount +=
                            numbro(msg.value.amount[a].amount).format("0,0") +
                            " " +
            ...
            return (
                <p>
                    ...
                    <em className="text-success">{amount}</em>{" "}
                    <T>activities.to</T>{" "}
            ...
            return (
                <p>
                    ...
                    <em className="text-warning">
                        {numbro(msg.value.amount.amount).format("0,0")}{" "}
        ...
        case "cosmos-sdk/MsgSubmitProposal":
            return (
                <p>
                    ...
                    <Link to={"/proposals/" + this.props.tags[2].value}>
                        {msg.value.title}
                    </Link>
                    <T>common.fullStop</T>
                </p>
                    ...
                    <em className="text-info">
                        {msg.value.amount.map((amount, i) => {
                            if (i > 0) {
                                return (
                                    " ," +
                                        numbro(amount.amount).format("0,0") +
                                        " " +
                                        amount.denom
                                );
                            } else {
        ...
        case "cosmos-sdk/MsgVote":
            return (
                <p>
                    ...
                    <MsgType type={msg.type} />{" "}
                    <Link to={"/proposals/" + msg.value.proposal_id}>
                        <T>activities.proposal</T> {msg.value.proposal_id}
                    </Link>{" "}
